		        	...
		        	var myJSON = JSON.stringify(data);
	                   		...
	                   		if(myJSON.resultado == "1")
	                   		{
								...
								for (var i = 0; i < myJSON.tabla.length; i++){
							        ...
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['apellido_materno']) ? '' : myJSON.tabla[i]['apellido_materno']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['institucion']) ? '' : myJSON.tabla[i]['institucion']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['ani']) ? '' : myJSON.tabla[i]['ani']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['folio']) ? '' : myJSON.tabla[i]['folio']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['fecha']) ? '' : myJSON.tabla[i]['fecha']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro font-weight-bold', (myJSON.tabla[i]['rc_resultado'] == '1' ? 'text-success' : (myJSON.tabla[i]['rc_resultado'] == '2' ? 'text-danger' : (myJSON.tabla[i]['rc_resultado'] == '3' ? 'text-info' : 'text-warning'))), '">', (jQuery.isEmptyObject(myJSON.tabla[i]['nombre_resultado_rc']) ? '' : myJSON.tabla[i]['nombre_resultado_rc']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro font-weight-bold', (myJSON.tabla[i]['id_certificado'] == '1' ? 'text-success' : (myJSON.tabla[i]['id_certificado'] == '2' ? 'text-danger' : 'text-warning')), '">', (jQuery.isEmptyObject(myJSON.tabla[i]['certificado']) ? '' : myJSON.tabla[i]['certificado']), '</td>');
							        row = row.concat('\n<td class="text-center align-middle registro">', (jQuery.isEmptyObject(myJSON.tabla[i]['via_entrada']) ? '' : myJSON.tabla[i]['via_entrada']), '</td>');
						            row = row.concat('\n</tr>');
